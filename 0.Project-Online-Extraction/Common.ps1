...
function Get-AADAuthResult ([Uri] $Uri, [string] $Region) {
    ...
    if ($Uri.Host -like "*.spoppe.com" -or $Uri.Host -like "*.spgrid.com") {
        # for eDog or Sandbox
...
function GetAuthCookie([Uri] $Uri, [string] $Region) {
    ...
    if ($authResult -ne $null) {
        ...
        $Response = Invoke-WebRequest -Headers $headers -Method "POST" -Body $Body -Uri $OAuthUri -UseBasicParsing
...
function CallOData([UserDataSoapClient] $Proxy, [string] $BaseUrl, [string] $Request, [bool] $OnPrem) {    
    ...
    $oDataBaseUri = New-Object Uri -ArgumentList @($baseUri, "_api/projectdata/[en-us]/")
...
function SaveODataAllPages([UserDataSoapClient] $Proxy, [string] $BaseUrl, [string] $Request, [string] $OutputDirectory, [string] $OutputFileName, [bool] $OnPrem) {
    ...
    Set-Content -LiteralPath $fullfileName -Value "{ ""all"" : [ " -Encoding Unicode -NoNewline
    # OG code - fails when file name has special chars like '['
    # "{ ""all"" : [ " > "$fullfileName"
    ...
    do {
        if ($pageCount -gt 1) {
            Add-Content -LiteralPath $fullfileName -Value ", " -Encoding Unicode -NoNewline
            # OG code - fails when file name has special chars like '['
            # ", " >> "$fullFileName"
        }
        Add-Content -LiteralPath $fullfileName -Value ($ret | ConvertTo-Json) -Encoding Unicode -NoNewline
        # OG code - fails when file name has special chars like '['
