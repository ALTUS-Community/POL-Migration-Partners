...
function Resolve-RelativePath {
    <#
    .SYNOPSIS
    Resolves paths starting with "." to absolute paths relative to a base folder.
    
    .DESCRIPTION
    Converts relative paths (starting with ".") to absolute paths based on the
    provided base folder. If the path is already absolute or doesn't start with ".",
    it is returned unchanged.
    
    .PARAMETER Path
    The path to resolve. Can be relative (starting with ".") or absolute.
    
    .PARAMETER BasePath
    The base folder to use for resolving relative paths. Typically the script's directory.
    
    .EXAMPLE
    Resolve-RelativePath -Path ".\Output" -BasePath "C:\Scripts"
    Returns: "C:\Scripts\Output"
    
    .EXAMPLE
    Resolve-RelativePath -Path "..\POLExports" -BasePath "C:\Scripts\SharePoint"
    Returns: "C:\Scripts\POLExports"
    #>
    param(
    ...
    # Return null/empty if path is not provided
    if ([string]::IsNullOrWhiteSpace($Path)) {
        return $Path
    }
    
    # If path starts with ".", resolve it relative to BasePath
    if ($Path -match '^\.') {
        $resolvedPath = Join-Path $BasePath $Path
        return [System.IO.Path]::GetFullPath($resolvedPath)
    }
    
    # Otherwise return as-is (already absolute or uses different syntax)
    return $Path
}
...
function Invoke-DateTimeFieldHandler {
    ...
    if ($Value) {
        ...
        # Format with Z suffix: yyyy-MM-ddTHH:mm:ssZ
        return $dtUtc.ToString("yyyy-MM-ddTHH:mm:ssZ")
    }
    return $null
}

function Invoke-DateOnlyFieldHandler {
    param($Value)
    ...
    if ($Value) {
        ...
        # If the datetime is UTC, convert to local time to get the correct local date
        if ($dt.Kind -eq [System.DateTimeKind]::Utc) {
            $dt = $dt.ToLocalTime()
        }
        
        # Extract just the date portion and set to midnight
        $dtZeroed = $dt.Date
                
        # Format as ISO 8601 with Z suffix: yyyy-MM-ddTHH:mm:ssZ
        return $dtZeroed.ToString("yyyy-MM-ddTHH:mm:ss") + "Z"
    }
    return $null
}
...
function Invoke-DecimalFieldHandler {
    param(
        $Value,
        $ColumnConfig
    )
    ...
    # Apply multiplier if specified
    if ($ColumnConfig -and $ColumnConfig.multiplier) {
        $decValue = $decValue * [decimal]$ColumnConfig.multiplier
    }
    
    # Apply divider if specified
    if ($ColumnConfig -and $ColumnConfig.divider) {
        if ([decimal]$ColumnConfig.divider -ne 0) {
            $decValue = $decValue / [decimal]$ColumnConfig.divider
        }
    }
...
# Detect modern SharePoint Image column payloads so we can skip them
function Test-IsSharePointImageValue {
    param($Value)

    if ($null -eq $Value) { return $false }

    $hasProps = $Value.PSObject.Properties.Name
    if ($hasProps -contains "fileName" -and $hasProps -contains "originalImageName") {
        return $true
    }

    if ($Value -is [string]) {
        $trimmed = $Value.Trim()
        if ($trimmed.StartsWith("{") -and $trimmed -match '"fileName"\s*:' -and $trimmed -match '"originalImageName"\s*:') {
            return $true
        }
    }
...
function Convert-SpItemToEntity {
    ...
    foreach ($c in $Ctx.columnMap) {
        ...
        if (Test-IsSharePointImageValue -Value $val) {
            Write-LogMessage "Skipping image column '$spName' for entity '$EntityLogicalName' (not supported)."
            continue
        }
        ...
        switch ($type) {
            ...
            "DateOnly" {
                $convertedValue = Invoke-DateOnlyFieldHandler -Value $val
            }
            { $_ -eq "Money" -or $_ -eq "Number" } {
                $convertedValue = Invoke-NumericFieldHandler -Value $val -ColumnConfig $c
            }
            "Decimal" {
                $convertedValue = Invoke-DecimalFieldHandler -Value $val -ColumnConfig $c
            }
            default {
    ...
    if ($Ctx.backlinkAttribute -and $Ctx.ItemUrl) {
        if ($null -ne $SchemaFieldLookup -and $SchemaFieldLookup -contains $Ctx.backlinkAttribute) {
