...
param(
    ...
    [string] $SiteCollectionUrl = "https://contoso.sharepoint.com/sites/pwa",
    ...
    [Parameter(Mandatory = $false)]
    [bool] $ExcludeVersionHistory = $true,
...
Write-LogMessage "AzureEnvironment:         $AzureEnvironment" -ForegroundColor Gray
...
Write-Host "  [OK] Documents and folder hierarchy from document libraries" -ForegroundColor Gray
Write-Host "  [OK] Basic file metadata (Name, Path, Size, Created, Modified, etc.)" -ForegroundColor Gray
Write-Host "  [OK] Version history for each document" -ForegroundColor Gray
Write-Host ""
Write-Host "What IS NOT exported:" -ForegroundColor White
Write-Host "  [NOT EXPORTED] Lists and list items (Risks, Issues, Tasks, etc.)" -ForegroundColor DarkGray
Write-Host "  [NOT EXPORTED] Web part pages, modern site pages, wiki pages" -ForegroundColor DarkGray
Write-Host "  [NOT EXPORTED] Picture libraries" -ForegroundColor DarkGray
Write-Host "  [NOT EXPORTED] Custom lists" -ForegroundColor DarkGray
Write-Host "  [NOT EXPORTED] Workflows and workflow approval history" -ForegroundColor DarkGray
Write-Host "  [NOT EXPORTED] Site settings, permissions, and configurations" -ForegroundColor DarkGray
Write-Host "  [NOT EXPORTED] Versions of pages" -ForegroundColor DarkGray
Write-Host "  [NOT EXPORTED] Custom metadata beyond basic file properties" -ForegroundColor DarkGray
Write-Host ""
...
try {
    ...
    try {
        Connect-PnPOnline -Url $SiteCollectionUrl -ClientId $ClientId -AzureEnvironment $AzureEnvironment -Interactive -ErrorAction Stop
    ...
    foreach ($web in $filteredWebs) {
        ...
        try {
            ...
            Connect-PnPOnline -Url $webUrl -ClientId $ClientId -AzureEnvironment $AzureEnvironment -Interactive
